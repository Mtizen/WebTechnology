			...
			<div class="table-wrapper">
				<div class="table-title">
					<div class="row">
						...
						<div class="col-sm-6">
							<a href="{{ route('home.create') }}" class="btn btn-success rounded">
				...
				<table class="table table-striped table-hover">
					...
					@foreach ($issues as $issue)
						...
						<td>{{ optional($issue->computers)->computer_name ?? 'Không có máy tính' }}</td>
						<td>{{ optional($issue->computers)->operating_system ?? 'Không có hệ điều hành' }}</td>
						...
						<td>{{ $issue->reported_date }}</td>
							...
							<td>
								<a href="{{ route('home.edit', $issue->id) }}">
									<i class="material-icons" data-toggle="tooltip" title="Edit" style="color: #FFC107; font-size: 28px;">&#xE254;</i>
								</a>

								<form action="{{ route('home.destroy', $issue->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa vấn đề này không?')">
									@csrf
									@method('DELETE')
									<button type="submit" class="btn btn-link p-0" style="border: none; background: none;">
										<i class="material-icons" data-toggle="tooltip" title="Delete" style="color: #F44336; font-size: 28px;">&#xE872;</i>
									</button>
								</form>
							</td>
						</tr>
